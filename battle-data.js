(function (root, factory) {
  ...
  const BATTLES = {
    ...
    "BT-03": {
      ...
      dialogueLines: {
        d1: "君らは秩序の名前で人を縛ってるだけだ。回せない側を、俺は待ち時間から出したい。",
      ...
      script: {
        open: [
          sp("ケイスケ", "先に言っておく。俺は誰も殴ってない。書類を減らしただけだ。——それで世界が困るなら、困る側の設計が間違ってる"),
          sp("リコ", "その理屈ごと、止めに来た")
        ],
        interrupt: {
          sync: [
        ...
        dialogue: {
          d1: [
            ...
            sp("ケイスケ", "回せない側は毎晩、書けない書類と鳴らない電話の前で削られてる。俺はそこから全員を出したいだけだ。——間違ってるか？"),
        ...
        retreatByResolution: {
          dialogue: [sp("ケイスケ", "……バグ報告、初めてもらったな。ユーザーがいたんだ、俺のツールに")]
        },
        forced: [
          sp("機械音声", "最終同期、完了シマシタ"),
    ...
    "BT-C2-02": {
      ...
      target: "代行機群", background: "bg_city_crossing_dusk", asset: "ryousan_clean",
      turnLimit: 8, resolutions: ["destroy", "control", "dialogue", "avoid"], timeout: "avoid",
      ...
      script: {
        ...
        after: [
          ...
          stg("現着カット。中枢の防壁が、すでに半分剥がされている。その前に、小柄な人影と大きなロボの影——")
        ]
      }
    },
    "BT-C2-03": {
      ...
      dialogueLines: {
        d1: "EXは俺の思想じゃない。意思確認を全部削って、便利だけに三万人が拍手した。",
      ...
      script: {
        open: [
          sp("ケイスケ", "作者の責任で、俺が消す。お前らに任せると、本家ごと悪者にされるだろ"),
        ...
        dialogue: {
          d1: [
            sp("ケイスケ", "言っておくが、EXは俺の思想じゃない。俺は『本当に、いいですか』を三回聞くんだ。三回だぞ。それを全部削って、便利だけ残して、三万人が拍手した"),
        ...
        resolution: {
          destroy: [
            ...
            sp("ケイスケ", "……俺が消したかったんだけどな、それ。まあいい。消えたなら、目的は同じ——"),
            sp("ケイスケ", "——じゃ、ないんだよな。たぶん")
          ],
          control: [
            stg("ノリとリコが中枢を掌握。ケイスケの手が届く前に、EXが静かに待機状態へ落ちる"),
            sp("ケイスケ", "……取ったのか。俺より先に、俺のコピーを"),
      ...
      script: {
        ...
        resolution: {
          ...
          dialogue: [
            ...
            stg("レントンの型（本章1回のみ）。レントンが Ver.1.0中枢に、ケイスケが読み込んだ住民の「体のデータ」を身体の言葉で突き合わせる"),
            sp("レントン", "あんたの『快適判定』、体のほうと突き合わせてみ。歩幅も、握力も、返事の速さも、みんな細うなっとる。……体が先に、答えを出しとるんよ"),
            stg("身体の話をされたとき、ケイスケの視線が一瞬だけ稼働データのウィンドウへ流れて、戻る"),
            stg("「離脱条項」自動発動。ケイスケ自身の手で、Ver.1.0中枢に「審判」と「離脱ボタン」が実装される経路が開く"),
            sp("ノリ", "離脱条項、適用。……『止めない。ただし出口を作る』。世界規模のVer.1.0に、退出のボタンが、いま初めて実装されます")
          ]
        },
        after: [
          stg("中枢の処遇が決まる。全編6パラメータの蓄積と各章の幕引き履歴が、1回の最終判定へ集約される")
        ],
        forced: [
          sp("機械音声", "最終同期、完了シマシタ"),
          sp("リコ", "引く！ 全員、市民側へ——……止めきれなかった。Ver.1.0、走り出した。せめて、巻き込まれる系統から、人を逃がす！"),
          stg("RAML、退避を優先して離脱。ケイスケが一人、統一されていく世界のモニタを見上げているカット（レントン、その横顔を最後に一度見る）")
        ]
      }
    }
  };
